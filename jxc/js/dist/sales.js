	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			var f = '<!--<a id="original" class="ui-btn ui-btn-sc">选择源单</a>--><a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				g = '<!--<a id="original" class="ui-btn ui-btn-sc">选择源单</a>--><a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invSa/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a><a href="../scm/invSa/toFahuodan?action=toFahuodan&id=' + a.id + '" target="_blank" id="printfhd" class="ui-btn">打印送货单</a><a id="edit" class="ui-btn">保存</a><a id="savaAndAddcopy" class="ui-btn ui-btn-sp" title="保存当前数据并复制当前数据为新单据" alt="保存当前数据并复制当前数据为新单据">保存复制</a>',
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				d && c.ajaxPost("../scm/invSa/addNew?action=addNew", {
					...
					if (200 === b.status) {
						a.$_number.text(b.data.billNo), $("#grid").clearGridData(), $("#grid").clearGridData(!0); //清除原有数据 tea
			...
			}), $(".wrapper").on("click", "#savaAndAddcopy", function(b) { //add by tea
				b.preventDefault();
				var c = $(this),
					d = THISPAGE.getPostData();
				d && c.ajaxPost("../scm/invSa/addNewcopy?action=addNewcopy", {
					postData: JSON.stringify(d)
				}, /*function(b) {
					if (200 === b.status) {
				...
				})*/ //保留GRID数据新增会出现问题，下面改成跟修改一样的处理方式// add by tea
				function(b) {
						200 === b.status ? (a.$_number.text(b.data.billNo),a.$_modifyTime.html((new Date).format("yyyy-MM-dd hh:mm:ss")).parent().show(), originalData.id = b.data.id, parent.Public.tips({
							content: "保存并复制新增成功！"
						})) : parent.Public.tips({
							type: 1,
							content: b.msg
			...
			}),$(".wrapper").on("click", "#printfhd", function(a) {
				a.preventDefault(), Business.verifyRight("SA_PRINT") && Public.print({
					title: "发货单列表",
					$grid: $("#grid"),
					pdf: "../scm/invSa/toFahuodan?action=toFahuodan",
					billType: 10201,
					filterConditions: {
						id: originalData.id
					}
