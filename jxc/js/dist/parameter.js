	...
	THISPAGE = {
		...
		initDom: function() {
			var a = [{
				...
				b = !0;
			if (this.$_companyName = $("#companyName"), this.$_companyAddress = $("#companyAddress"), this.$_companyTel = $("#companyTel"), this.$_companyFax = $("#companyFax"),this.$_companyVchat = $("#companyVchat"),this.$_companyMail = $("#companyMail"), this.$_postcode = $("#postcode"), this.$_startDate = $("#startDate"), this.$_currency = $("#currency"), this.$_qtyPlaces = $("#qtyPlaces"), this.$_pricePlaces = $("#pricePlaces"),this.$_halfpageNo = $("#halfpageNo"),this.$_pageNo = $("#pageNo"), this.$_amountPlaces = $("#amountPlaces"), this.$_valMethods = $("#valMethods"), this.$_requiredCheckStore = $("#requiredCheckStore"), this.$_establishForm = $("#establish-form2"), this.$_companyName.val(system.companyName), this.$_companyAddress.val(system.companyAddr), this.$_companyTel.val(system.phone), this.$_companyFax.val(system.fax), this.$_postcode.val(system.postcode), this.$_startDate.val(system.startDate), this.$_currency.val(system.currency), this.$_qtyPlaces.spinbox({
				value: system.qtyPlaces,
				width: 80
			}), this.$_pricePlaces.spinbox({
				value: system.pricePlaces,
				width: 80
			}), this.$_halfpageNo.spinbox({
				value: system.halfpageNo,
				width: 80
			}), this.$_pageNo.spinbox({
				value: system.pageNo,
				width: 80				
			...
			$("#save").click(function() {
				if (Business.verifyRight("SYSTEM_UPDATE")) {
					var b = $.trim(a.$_companyName.val()),
						...
						v = $.trim(a.$_companyVchat.val());
						m = $.trim(a.$_companyMail.val());
					...
					var g = {
						...
						halfpageNo: a.$_halfpageNo.val(),
						pageNo: a.$_pageNo.val(),
						amountPlaces: 2,
